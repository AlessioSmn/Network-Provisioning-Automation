...
To avoid unintended traffic being forwarded toward the management network, we configured the startup scripts of all nodes to optionally move the management interface into a dedicated and isolated VRF.
To allow us to avoid that default route and consequent traffic towards the management network, we configured the startup files of all nodes to be able to move the management interface into a dedicated and separated VRF. If the environment variable \monospaced{CLAB\_MGMT\_VRF} is specified in the topology file, as represented in Listing \ref{lst:mgmt_vrf_topo},
\begin{lstlisting}[style=styleYml, caption=Management VRF in acn.clab.yml, label=lst:mgmt_vrf_topo,
...
The Alpine dockerfile is found at \monospaced{./config/alpine/dockerfile}.
...
The Manager dockerfile is found at \monospaced{./config/mngr/dockerfile}.
...
As per project specification the traffic load balancing via BGP attributes is done only for the two upstream links (GW1-UP1 and GW2-UP2).\\
...
Moreover, a file called \texttt{traffic-matrices.json} is passed to the manger as well. In this json file, the traffic estimation (Gb/s) for each route is included. An exmaple of a traffic matrix structure is shown in Listing \ref{lst:traffic-matrices}:
...
\subsection{Resulting tables}
\subsubsection{Matrix 1 - resulting tables}
...
\subsubsection{Matrix 2 - resulting tables}
...
\subsection{Traceroute examples}
...
\end{figure}
\begin{itemize}
    \item Matrix1: n1, CE1, PE1, GW1, UP1, INT, n3 
    \item Matrix2: n1, CE1, PE1, GW1, UP1, INT, n3 
\end{itemize}

\subsubsection{n2 to n4}
\begin{figure}[H]
    \centering
...
\end{figure}
\begin{itemize}
    \item Matrix1: n2, CE2, PE2, GW2, UP2, INT, n4 
    \item Matrix2: n2, CE2, PE2, GW2, UP2, INT, n4 
\end{itemize}

\subsubsection{n3 to n1}
\begin{figure}[H]
    \centering
...
\end{figure}
\begin{itemize}
    \item Matrix1: n3, INT, UP2, GW2, PE2, CE1, n1
    \item Matrix2: n3, INT, UP2, GW2, PE2, CE1, n1
\end{itemize}

\subsubsection{n4 to n2}
\begin{figure}[H]
    \centering
...
\end{figure}
\begin{itemize}
    \item Matrix1: n4, INT, UP1, GW1, PE1, CE2, n2
    \item Matrix2: n4, INT, UP2, GW2, PE1, CE2, n2
\end{itemize}
...
\section{Future work}
Despite these positive results, several aspects of the project can be further improved, particularly with respect to the load balancing strategy.
\paragraph{Route maps}
In the current implementation, route maps are periodically removed and re-created every three minutes. 
...
\paragraph{Expansion}
The current approach used to achieve a sort of load balancing across the two upstream links can be replicated and used to balance traffic on the customer side of the network. We choose not to implement this as only the upstream link were required and it would be conceptually very similar, just increasing the complexity of the algorithm.
