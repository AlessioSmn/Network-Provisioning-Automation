...
If instead the \monospaced{bgp} field is present, the template configures the router-id (field \monospaced{ID}) and the current router's ASN (field \monospaced{AS}); also two other fields are needed for the neighbors configuration:
\begin{itemize}
...
def configure_filtering(gw_ip, route_in_map_name, bucket):
    ...
    for peer_ip in ALL_PEERS:
        ...
        cmds.append(f' neighbor {peer_ip} route-map {route_in_map_name} in')
...
\todo{show differences with matrix1 and matrix2: bgp, route-tables and traceroute}

\chapter{Conclusion}
\todo{Parte potenzialmente molto inutile}
\todo{Dire che il ciclo di prevelare matrice di traffico e applicare l'algoritmo è fatto ogni X minuti, ma è chiaramente a scopi dimostrativi di testing. Una implementazione più seria prendere i dati da stime sui dati di traffico + possibli previsioni su altre dati, e probabilmente sarebbe aggiornato solo se necessario e meno frequentemente.}
